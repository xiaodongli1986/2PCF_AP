...
! Setting: directory of the 2pCF (2-point correlation funtion) files 
  character(len=charlen), parameter :: filedir = '/home/xiaodongli/LSS/2PCF_AP/'
...
! data (baseline)    
  character(len=charlen) function data2pcffile_base(iz, ombase, wbase)
    ...
    if (iz .eq. 1) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_LOWZ_data.xyzw.1of3.cosmo-converted.'//&
	trim(adjustl(omwstr(ombase,wbase)))//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 2) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_LOWZ_data.xyzw.2of3.cosmo-converted.'//&
	trim(adjustl(omwstr(ombase,wbase)))//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 3) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_LOWZ_data.xyzw.3of3.cosmo-converted.'//&
	trim(adjustl(omwstr(ombase,wbase)))//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 4) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_CMASS_data.xyzw.1of3.cosmo-converted.'//&
	trim(adjustl(omwstr(ombase,wbase)))//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 5) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_CMASS_data.xyzw.2of3.cosmo-converted.'//&
	trim(adjustl(omwstr(ombase,wbase)))//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 6) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_CMASS_data.xyzw.3of3.cosmo-converted.'//&
	trim(adjustl(omwstr(ombase,wbase)))//trim(adjustl(filestr))//'2pcf'
    endif
  ...
  character(len=charlen) function data2pcffile_base_J08(iz)
    ...
    filestr = '.rmax'//trim(adjustl(tmpstr3))//'.'//trim(adjustl(tmpstr1))//'rbins.'&
      //trim(adjustl(tmpstr2))//'mubins.'
    if (iz .eq. 1) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_J08.RSD.002.xyzw.1of3'//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 2) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_J08.RSD.002.xyzw.2of3'//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 3) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_J08.RSD.002.xyzw.3of3'//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 4) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_J08.RSD.002.xyzw.1of3'//trim(adjustl(filestr))//'2pcf'
    elseif (iz .eq. 5) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_J08.RSD.002.xyzw.2of3'//trim(adjustl(filestr))//'2pcf'
...
! mock (for systematic correction)
  character(len=charlen) function syscor2pcffile(iz, imock)
    ...
    if (iz .eq. 1) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_LOWZ_'//trim(adjustl(str1))//&
    ...
    elseif (iz .eq. 4) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_CMASS_'//trim(adjustl(str1))//&
                               '.xyzw.1of3.rmax150.150rbins.120mubins.2pcf'
    elseif (iz .eq. 5) then
      filename = trim(adjustl(filedir))//'/2pcfs/DR12_CMASS_'//trim(adjustl(str1))//&
                               '.xyzw.2of3.rmax150.150rbins.120mubins.2pcf'

...
%DIF DEL Draft1.2.tex   Thu Aug  3 12:23:02 2017
%DIF ADD Draft1.4.tex   Mon Aug  7 14:06:42 2017
%%
...
\author{Xiao-Dong Li,}
\DIFdelbegin %DIFDELCMD < \affil{School of Physics, Korea Institute for Advanced Study, 85 Heogiro, Dongdaemun-gu, Seoul 130-722, Korea}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil{School of Physics, Korea Institute for Advanced Study, 85 Heogiro, Dongdaemun-gu, Seoul 02455, Korea}
\DIFaddend \author{Cristiano G. Sabiu,}
\affil{Korea Astronomy and Space Science Institute, 776, Daedeokdae-ro, Yuseong-gu, Daejeon, 34055, Korea}
\author{Changbom Park,}
\DIFdelbegin %DIFDELCMD < \affil{School of Physics, Korea Institute for Advanced Study, 85 Heogiro, Dongdaemun-gu, Seoul 130-722, Korea}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil{School of Physics, Korea Institute for Advanced Study, 85 Heogiro, Dongdaemun-gu, Seoul 02455, Korea}
\DIFaddend \author{Yuting Wang, Gong-bo Zhao}
...
\DIFdelbegin %DIFDELCMD < \altaffiltext{1}{Corresponding Author: kjhan@kias.re.kr}
%DIFDELCMD < %%%
\DIFdelend %DIF > \altaffiltext{1}{Corresponding Author: kjhan@kias.re.kr}
...
\begin{abstract}
We \DIFdelbegin \DIFdel{use the anisotropic clustering signal of }\DIFdelend \DIFaddbegin \DIFadd{perform the Alcock-Paczynski (AP) test via the redshift dependence of clustering on scales of 6--40 $h^{-1}\rm Mpc$
to place constraints on the expansion history of the Universe.
We use }\DIFaddend 1,133,326 galaxies \DIFdelbegin \DIFdel{from the }\DIFdelend Sloan Digital Sky Survey Data Release 12 \DIFaddbegin \DIFadd{galaxies}\DIFaddend , spanning the redshift range $0.15<z<0.69$\DIFdelbegin \DIFdel{to place constraints on the expansion history of the Universe}\DIFdelend .
...
\DIFdelbegin \DIFdel{The Alcock-Paczynski (AP) test was performed via the redshift dependence of clustering on scales of 6-40 $h^{-1}\rm Mpc$.
}\DIFdelend We improve the methodology of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2016} }%DIFAUXCMD
by significantly reducing its computation time. This enables us }\DIFdelend \DIFaddbegin \DIFadd{Li et al. }\DIFaddend to efficiently explore three or more cosmological parameters.
We then apply the improved method on the Chevallier-Polarski-Linder parametrization of dark energy, $w=w_0+w_a\frac{z}{1+z}$.
...
We checked the robustness of the results.
...
%DIF > \begin{figure*}
%DIF >    \centering{
%DIF >    %\includegraphics[width=9cm,natwidth=4,natheight=4]{figCPL_a.pdf}
%DIF >    \includegraphics[height=6.5cm]{fig_nbar.eps}
%DIF >    \includegraphics[width=9.0cm,]{fig_2pcf.eps}
%DIF >    }
%DIF >    \caption{\label{fig_TpCF}
%DIF >    Splitting the SDSS galaxies into six redshift bins, and measuring the correlation function
%DIF >    (similar figures already presented in \cite{Li2016}).
%DIF >    Left panel: We split the BOSS DR12 galaxies into six redshift bins to measure the evolution of shape distortion.
%DIF >    The blue and green solid histograms show the redshift density distribution of LOWZ and CMASS galaxies in the $\Omega_m=0.31$ $\Lambda$CDM. 
%DIF >    The vertical dashed lines define the six redshift bins that are used to cut the samples.
%DIF >    The number of LOWZ/CMASS galaxies in the three low/high redshift bins are listed.
%DIF >    Right panel: 2D contour map of measured $\xi$ as a function of $\mu$ and $s$, from the six redshift bins of LOWZ and CMASS samples, 
%DIF >       in the cosmology of $\Omega_m=0.31$ $\Lambda$CDM model.
%DIF >     The black dashed lines mark the clustering region 6-40\ Mpc/h studied in the AP method.
%DIF >     The six contour maps have rather similar appearance, implying small redshift evolution of $\xi$ from RSD.
%DIF >    }
%DIF > \end{figure*}
\DIFaddbegin 
...
\begin{figure*}
   ...
   %DIF > We split the BOSS DR12 galaxies into six redshift bins to measure the evolution of shape distortion.
   \DIFaddFL{A patch of BOSS DR12 glaxies located in the fan shape region $\rm 140^\circ < RA < 170 ^\circ$, $\rm 10^\circ < DEC < 13 ^\circ$.
   ...
   In the left and right panels, we plot the galaxies postions computed in two cosmologies: the $\Omega_m$=0.26 $\Lambda$CDM, 
   ...
   %DIF > The shape of structures look different in these two cosmologies. 
   In the dynamical dark energy cosmology, we see apprent relative stretch of structures compared with the $\Lambda$CDM cosmology.
   ...
   %DIF > By investigating how the anosotropy of galaxy distribution evolves in the six redshift bins,
   %DIF > we are able to distinguish different cosmologies.
   %DIF > The obtained galaxy distribution is different.
   %DIF > 
   %DIF > The blue and green solid histograms show the redshift density distribution of LOWZ and CMASS galaxies in the $\Omega_m=0.31$ $\Lambda$CDM. 
   %DIF > The vertical dashed lines define the six redshift bins that are used to cut the samples.
   %DIF > The number of LOWZ/CMASS galaxies in the three low/high redshift bins are listed.
...
to modifications of Einstein's General Relativity \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{2012IJMPD..2130002Y}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{2012IJMPD..2130002Y}}%DIFAUXCMD
}\DIFaddend . 
...
The symmetry properties of galaxy pairs\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Marinoni2010}  }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Marinoni2010}  }%DIFAUXCMD
}\DIFaddend could also be used to probe the AP effect;
...
In an effort to minimize RSD contamination, the shape of void regions\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ryden1995,LavausWandelt1995}  }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Ryden1995,LavausWandelt1995}  }%DIFAUXCMD
}\DIFaddend has been 
...
Previously, we proposed to use the {\it redshift dependence} of the AP distortion\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2014} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Li2014} }%DIFAUXCMD
}\DIFaddend as a way of mitigating the RSD effect. 
...
(as an illustration, \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig_xy} shows how the shape distortion of objects varies with distance (i.e., redshift) 
when incorrect cosmologies are used to infer distances).
In \DIFdelbegin \DIFdel{a later work}\DIFdelend \cite{Li2015}, we developed an AP methodology 
...
\DIFaddbegin \begin{figure*}
   ...
   \DIFaddFL{$\hat\xi_{\Delta s}(\mu)$ measured from BOSS DR12 galaxies, in six redshift bins (three in LOWZ and three in CMASS).
...
However, as a first step, in this paper we will focus on the widely used Chevallier-Polarski-Linder (CPL) parametrization\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CPL_CP,CPL_L}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{CPL_CP,CPL_L}}%DIFAUXCMD
}\DIFaddend ,
\begin{equation}
...
and the ability to accurately reconstruct many dark energy theories\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CPL_L}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{CPL_L}}%DIFAUXCMD
}\DIFaddend . 
So it was widely used in the literature to reconstruct dark energy from observations.
The constraining power is usually quantified by the DETF (Dark Energy Task Force) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{DETF} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{DETF} }%DIFAUXCMD
...
\subsection{\DIFadd{Quantifying the redshift dependece of AP distortion}}

\DIFaddend The spectroscopic galaxy sample of SDSS-III BOSS (Baryon Oscillation Spectroscopic Survey) has two primary catalogues:
...
targeted to be an approximately stellar mass limited sample of massive, luminous galaxies \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Reidetal:2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Reidetal:2016}}%DIFAUXCMD
}\DIFaddend .
...
The \DIFdelbegin \DIFdel{number density of galaxies and the redshift binning scheme is plotted in the left panel of Fig. 
\ref{fig_TpCF}.
}\DIFdelend \DIFaddbegin \DIFadd{edges are determined so that the number of galaxies are roughly the same in different redshift bins 
(for LOWZ and CMASS samples, respectively).
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD <    \centering{
%DIFDELCMD <    %\includegraphics[width=9cm,natwidth=4,natheight=4]{figCPL_a.pdf}
%DIFDELCMD <    \includegraphics[height=6.5cm]{fig_nbar.eps}
%DIFDELCMD <    \includegraphics[width=9.0cm,]{fig_2pcf.eps}
%DIFDELCMD <    }
%DIFDELCMD <    %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig_TpCF}
%DIFDELCMD <    %%%
\DIFdelFL{Splitting the SDSS galaxies into six redshift bins, and measuring the correlation function
   (similar figures already presented in \mbox{%DIFAUXCMD
\cite{Li2016}}%DIFAUXCMD
).
...
%DIFDELCMD < %%%
\DIFdelend %DIF > The number density of galaxies and the redshift binning scheme is plotted in the left panel of Figure \ref{fig_TpCF}.
\DIFaddbegin 

\DIFadd{Figure \ref{fig_fan} shows the positions of a patch of 10,976 BOSS DR12 glaxies,
...
In the later cosmology, there is apprent stretch of structures, 
...
%DIF < \footnote{These correlations were computed using the public code \texttt{KSTAT} \citep{kstat}.} 
as 
...
%DIFDELCMD < %%%
\DIFdel{The right panel of Fig.\ref{fig_TpCF} shows the contour map of $\xi(s,\mu)$ measured in the six redshift bins.
...
}\DIFdelend %DIF > The right panel of Figure \ref{fig_TpCF} shows the contour map of $\xi(s,\mu)$ measured in the six redshift bins.
%DIF > The contour lines are not horizontal due to the effects of peculiar velocity,
%DIF > and the RSD effect clearly manifest itself through the tilting of contour lines.
%DIF > However, the shape of the tilted lines remain similar at different redshifts, a fact that enables us  to mitigate the RSD effect by focusing on the redshift dependence of the anisotropic clustering signal.
...
For the removal of fiber collision and the strong \DIFdelbegin \DIFdel{FoG effect }\DIFdelend \DIFaddbegin \DIFadd{finger of god (FoG) effect \mbox{%DIFAUXCMD
\citep{FOG} }%DIFAUXCMD
}\DIFaddend near the LOS we take a cut $\mu_{\rm max} = 0.97$.

Systematics effects are estimated using mock catalogues derived from Horizon Run 4 (HR4) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{HR4}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{HR4}}%DIFAUXCMD
}\DIFaddend ,
...
\DIFaddbegin \DIFadd{Mock galaxy samples are produced using a modified version of the one-to-one correspondence scheme \mbox{%DIFAUXCMD
\citep{hong2016}}%DIFAUXCMD
. 
}\DIFaddend As presented in \cite{Li2016}, the value of $\left[\hat\xi_{\Delta s}(z_i,\mu_j)-\hat\xi_{\Delta s}(z_1,\mu_j)\right]_{\rm sys}$
...
\DIFdelbegin \DIFdel{Mock galaxy samples are produced using a modified version of the one-to-one correspondence scheme \mbox{%DIFAUXCMD
\citep{hong2016}}%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin 


\DIFaddend The covariance matrix, ${\bf Cov}$, is computed from the 2,000 sets of MultiDark PATCHY mock catalogues \citep{MDPATCHY}.
...
\DIFaddbegin \DIFadd{The MultiDark PATCHY  mocks are produced using approximate gravity solvers and analytical-statistical biasing models.
They were calibrated to the BigMultiDark N-body simulation \mbox{%DIFAUXCMD
\citep{K2014}}%DIFAUXCMD
, which 
...
%DIF > $\Omega_m = 0.307115$, $\Omega_b = 0.048206$, $\sigma_8 = 0.8288$, $n_s = 0.9611$, and $H_0 = 67.77 {\rm km}\ s^{-1} {\rm Mpc}^{-1}$.
...
They have been adopted for statistical analysis of BOSS data in a series of works (see \mbox{%DIFAUXCMD
\cite{Alam2016} }%DIFAUXCMD
...
%DIFDELCMD <    %\includegraphics[width=9cm,natwidth=4,natheight=4]{figCPL_a.pdf}
%DIFDELCMD <    \includegraphics[height=5cm,natwidth=6,natheight=4.5]{figCPL_b.pdf}
%DIFDELCMD <    %\includegraphics[width=5.5cm,natwidth=4,natheight=4]{figCPL_b_BAOSNIaAP.pdf}
%DIFDELCMD <    \includegraphics[height=5cm,natwidth=6,natheight=4.5]{figCPL_d.pdf}
%DIFDELCMD <    %\includegraphics[height=8cm]{fig2b.pdf}
%DIFDELCMD < %   \includegraphics[height=8cm]{Tpcf--plot--Normed.eps}
%DIFDELCMD < %    \includegraphics[height=8cm]{smu.eps}
%DIFDELCMD <    }
%DIFDELCMD <    %%%
...
%   \includegraphics[width=10.5cm]{fig_comp.eps}
   ...
   \DIFaddendFL \caption{\DIFdelbeginFL %DIFDELCMD < \label{fig_con}
%DIFDELCMD <    %%%
\DIFdelFL{Cosmological constraints on }\DIFdelendFL \DIFaddbeginFL \label{fig_method}
   \DIFaddFL{Mapping $\xi(s,\mu)$ from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{CPL dark energy parametrization $w=w_0+w_a\frac{z}{1+z}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fiducial cosmology (taken as the $\Omega_m=0.26$ $\Lambda$CDM cosmology) to other cosmologies}\DIFaddendFL . 
 \DIFdelbeginFL \DIFdelFL{Left panel: 68.3\%, 95.4\% CL likelihood contours }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The number counts are measured }\DIFaddendFL in the \DIFdelbeginFL \DIFdelFL{$w_0-w_a$ plane}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fiducial cosmology in the blue dashed grid}\DIFaddendFL .
 %DIF < Results are consistent with $\Lambda$CDM.
   \DIFdelbeginFL \DIFdelFL{The constrained area is significantly reduced after combined }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{In the four cosmologies }\DIFaddendFL with \DIFaddbeginFL \DIFaddFL{$(w_0,w_a) = (-1.5,0),\ (-0.5,0),\ (-1,2)\ \rm and\ (-1,-2)$,
 %DIF > due to the distortion of geometry,
 according to Equation \ref{eq:smumap},
 }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{AP method.
   %DIF < This shows the power of the AP method in constraining dynamical dark energy.
   %DIF < Middle panel: $w_0-w_a$ likelihood contours using datasets of CMB+BAO, CMB+BAO+SNIa+$H_0$ and CMB+BAO+AP+$H_0$.
   %DIF < When combined with CMB+BAO, AP method yields much tighter constraints than SNIa.
   Right panel: Redshift evolution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distribution }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{$w(z)$, with/without adding }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{measured number counts in $(s, \mu)$ space becomes }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{AP constraint.
   Adding AP tightens }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{red solid grid,
 To ensure }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{constraints and reduces }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{accuracy, we count }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{redshift evolution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{number }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{$w$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pairs in 5 times smaller pixels }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{tilt of $w(z)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the small red pixels}\DIFaddendFL )\DIFaddbeginFL \DIFaddFL{, 
 and group them together to infer the values of $\xi(s, \mu)$ in blue dashed pixels.
 For illustration purpose, 
  the blue and red grids are all 10 times sparser than the grids adopted in the real analysis}\DIFaddendFL .
}
\end{figure*}

%DIF < Covariance matrix 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{figure*}
   \DIFdelbeginFL %DIFDELCMD < \centering{
%DIFDELCMD <    \includegraphics[width=18cm,natwidth=6,natheight=5]{fig_con_tests.pdf}
%DIFDELCMD <    %\includegraphics[width=9cm,natwidth=4,natheight=4]{fig2b.pdf}
%DIFDELCMD <    %\includegraphics[height=8cm]{fig2b.pdf}
%DIFDELCMD < %   \includegraphics[height=8cm]{Tpcf--plot--Normed.eps}
%DIFDELCMD < %    \includegraphics[height=8cm]{smu.eps}
%DIFDELCMD <    }
...
\DIFdelendFL \DIFaddbeginFL \centering{
   ...
   \DIFaddendFL \caption{\DIFdelbeginFL %DIFDELCMD < \label{fig_contest}
%DIFDELCMD <    %%%
\DIFdelFL{Robustness }\DIFdelendFL \DIFaddbeginFL \label{fig_Check}
 \DIFaddFL{Left panel: Input-output }\DIFaddendFL test of the \DIFdelbeginFL \DIFdelFL{results.
   For }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{methodology using four sets of mock catalogues of BOSS DR12 galaxies constructed from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{``default'' constraints (pink contours) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{HR4 simulation.
 The approximate 2pCFs }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{derived using six redshift bins, 
   $\mu_{\rm max}=0.97$, $n_{\mu}=15-20$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{adopted in the analysis.
 The simulation cosmology}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$s=6-40 \rm Mpc/h$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Omega_m=0.26$ $\Lambda$CDM}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{a fiducial cosmology $\Omega_m=0.26,\ w=-1.0$ for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is marked by }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{measurement of 2pCF,
   systematic effects estimated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{black plus.
 The inferred cosmological constraints }\DIFaddendFL from \DIFdelbeginFL \DIFdelFL{Horizon Run 4 simulations}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the method}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{and covariance estimated using 2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{shown in the cygan contours}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{000 MultiDark PATCHY mocks}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are consistent with the input cosmology}\DIFaddendFL .
 \DIFdelbeginFL \DIFdelFL{When we alters one of these options by using $\mu_{\rm max}=0.99$, 
   $\mu_{\rm max}=0.85$, $n_{\mu}=15-20$, $s=8-30 \rm Mpc/h$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Right panel: Cosmological constraints from the BOSS DR12 galaxies}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{assuming }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{fiducial }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$w$CDM }\DIFaddendFL cosmology 
 \DIFdelbeginFL \DIFdelFL{of $\Omega_m=0.26,\ w=-0.6$, 
   excluding the last redshift bin in the analysis, 
   discarding the systematics correction}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(i.e.}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{or reducing }\DIFdelendFL the \DIFdelbeginFL \DIFdelFL{number }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{value }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{mocks in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$w_a$ fixed as zero).
 Results obtained using }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{estimation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{precise 2pCFs (cyan filled) 
 and approximate 2pCFs (red dashed) agree with each other quite well.
 The computation }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{covariance matrix}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{precise 2pCFs is very expensive}\DIFaddendFL ,
 \DIFaddbeginFL \DIFaddFL{so }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{results remain robust (black contours)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{comparison was done on a coarse grid of cosmological parameters with $\delta \Omega_m=0.025,\ \delta w=0.05$}\DIFaddendFL .
   }
\end{figure*}

\DIFaddbegin \DIFadd{As an illustration,
...
\DIFadd{The shape of $\hat \xi_{\Delta s}(\mu)$ is far away from a flat curve, 
due to the apparent anisotropy produced by the peculiar montion of galaxies.
...
\DIFadd{The green curves denote the 2pCFS measured from the HR4 mock catalogues
(we always plot the }{\it \DIFadd{correct}} \DIFadd{measurement in the simulation cosmology, i.e. the $\Omega_m=0.26$ $\Lambda$CDM).
%DIF > The small redshift evolution of $\hat \xi_{\Delta s}(\mu)$
The simulation results can well match the results from real observational data,
indicating that the FoG \mbox{%DIFAUXCMD
\citep{FOG} }%DIFAUXCMD
and Kaiser \mbox{%DIFAUXCMD
\citep{Kaiser1987} }%DIFAUXCMD
effects are both well reproduced.
...
\subsection{\DIFadd{Approximating the 2pCFs in cosmologies other than the fiducial one}}\label{sec:approx_2pcf}

\DIFaddend In our earlier work\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Li2016}}%DIFAUXCMD
}\DIFaddend , the likelihood contour of $\Omega_m-w$ was constructed by
...
This procedure took 1 month using 500 cores of the \DIFdelbegin \DIFdel{KIAS }\DIFdelend \DIFaddbegin \DIFadd{Korea Institute for Advanced Study }\DIFaddend {\texttt {baekdu}} cluster.
...
%DIF < The left panel of Figure \ref{fig_method} shows how 
%DIF < $\xi(s,\mu)$ is mapped from the fiducial cosmology to another cosmology using the above relations.
...
%DIF < in the fiducial cosmology and group the bins in the target cosmology.
...
\DIFdelbegin \DIFdel{.
%DIF < \footnote{A dense grid can effectively reduce the ``edge effect'' produced when we map $\xi(s,\mu)$ from one cosmology to another.
}\DIFdelend %DIF > in the fiducial cosmology and group the bins in the target cosmology.
\DIFaddbegin \footnote{\DIFadd{A denser grid can significantly reduce the edge effect;
if we use Equation \ref{eq:smumap} to do a simple interpolation on a grid with same resoultion does not work,
the edge effect is so large that the derived cosmological constraints suffer from a significant error.}}
\DIFaddend %The edge effect would introduce artificial correlations between nearby pixels, smooth $\xi(s,\mu)$ a little bit, 
...
\DIFdelbegin \DIFdel{We found when using the approximate method the computed 
$\hat\xi_{\Delta s}(\mu)$ in a non-fiducial cosmologysuffers from
an error of }\DIFdelend \DIFaddbegin \DIFadd{The above procedure is illustrated in Figure \ref{fig_method}.
...
%DIF > We checked that the approximate method can reproduce the results presented in \cite{Li2016} quite well.

\DIFadd{Figure \ref{fig_xi} plots the approximate 2pCFS in the $\Omega_m=0.11$ $\Lambda$CDM cosmology.
We find that, the approximation procedure only introduces a }\DIFaddend $\lesssim0.5\%$ \DIFaddbegin \DIFadd{error in the $\hat\xi_{\Delta s}(\mu)$}\DIFaddend , 
which is 10 times smaller than the \DIFdelbegin \DIFdel{statistical noise .
We checked }\DIFdelend \DIFaddbegin \DIFadd{noise of $\hat\xi_{\Delta s}(\mu)$.
So we expect }\DIFaddend that the approximate method can reproduce the results \DIFdelbegin \DIFdel{presented in \mbox{%DIFAUXCMD
\cite{Li2016} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{of precise method }\DIFaddend quite well.
%DIF <  We checked that the approximate method can reproduce the results of precise method quite well.
%DIF <  The right panel of Figure \cite{fig_method} shows the  $\Omega_m$-$w_0$ constraints obtained 
%DIF <   using the precise and approximate 2pCFs (here we fix $w_a=0$). 
%DIF <   They agree with each other quite well.


\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD <    \centering{
%DIFDELCMD <    \includegraphics[width=6cm]{fig_smu.eps}
%DIFDELCMD <    \includegraphics[width=10.5cm]{fig_comp.eps}
%DIFDELCMD <    }
%DIFDELCMD <    %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig_method}
%DIFDELCMD <    %%%
\DIFdelFL{Left panel: Mapping $\xi(s,\mu)$ from the fiducial cosmology to four other cosmologies. 
...
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{An input-output test was performed using the four sets of mock catalogues of BOSS DR12 galaxies constructed from HR4 \mbox{%DIFAUXCMD
\citep{HR4}}%DIFAUXCMD
.
...
\section{\DIFdelbegin \DIFdel{Result}\DIFdelend \DIFaddbegin \DIFadd{Cosmoligical constraints}\DIFaddend }


The Planck team has released the {\texttt {COSMOMC}} \citep{LB2002} outputs of four \DIFdelbegin \DIFdel{MCMC }\DIFdelend \DIFaddbegin \DIFadd{Markov Chain Monte Carlo (MCMC) 
}\DIFaddend ``chains'' in the CPL model, 
using a combination of four datasets:
the full-mission Planck observations of CMB temperature and polarization anisotropies \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Planck2015}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Planck2015}}%DIFAUXCMD
}\DIFaddend ,
...
and the Hubble Space Telescope measurement of $H_0=70.6\pm3.3$ km/s/Mpc \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Riess2011,E14H0}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Riess2011,E14H0}}%DIFAUXCMD
}\DIFaddend .
These MCMC chains contain the CMB+BAO+SNIa+$H_0$ likelihood computed for $\sim$37,000 sets of cosmological parameters\DIFdelbegin \DIFdel{,
yielding constraints of %DIF < $ \Omega_m = 0.309 \pm 0.010, w_0 = -0.938 \pm 0.109, w_a = -0.38 \pm 0.41$.
%DIF < \begin{eqnarray}
%DIF <  \Omega_m &=& 0.309 \pm 0.010,  \\ \nonumber
%DIF <  w_0 &=& -0.938 \pm 0.109,  \\ \nonumber
%DIF <  w_a &=& -0.38 \pm 0.41.
%DIF < \end{eqnarray}
}%DIFDELCMD < \small
%DIFDELCMD < %%%
\begin{displaymath}
 \DIFdel{\Omega_m = 0.309 \pm 0.010,\
 w_0 = -0.938 \pm 0.109,\
 w_a = -0.38 \pm 0.41.
}\end{displaymath}
%DIFAUXCMD
...
}\DIFaddend %To derive the constraints combined with our AP method,
...
%DIF < measured the 2pCF (on scales of 6-40 $h^{-1}$Mpc) in these redshift bins (see Fig.\ref{fig_nbar}).
%DIF > measured the 2pCF (on scales of 6-40 $h^{-1}$Mpc) in these redshift bins (see Figure \ref{fig_nbar}).
...
we derive the CMB+BAO+SNIa+$H_0$+AP constraints on CPL parameters\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend %$\Omega_m = 0.301 \pm 0.008, w_0 = -1.042 \pm 0.067, w_a = -0.07 \pm 0.29$.
%DIF < \begin{eqnarray}
%DIF < \Omega_m &=& 0.301 \pm 0.008, \\ \nonumber
%DIF < w_0 &=& -1.042 \pm 0.067, \\ \nonumber
%DIF < w_a &=& -0.07 \pm 0.29.
%DIF < \end{eqnarray}
\DIFdelbegin %DIFDELCMD < \small
%DIFDELCMD < %%%
\begin{displaymath}
 \DIFdel{\Omega_m = 0.301 \pm 0.008,\
 w_0 = -1.042 \pm 0.067,\
 w_a = -0.07 \pm 0.29.
}\end{displaymath}
%DIFAUXCMD
...
%    \includegraphics[height=8cm]{smu.eps}
   ...
   \DIFaddFL{Cosmological constraints on the CPL dark energy parametrization $w=w_0+w_a\frac{z}{1+z}$.
   ...
   Results from the AP method alone (brown filled), CMB+BAO (cygan filled), CMB+BAO+SNIa+$H_0$ (blue filled)
   ...
   %DIF > The AP method leads to competitive constraints on the cosmological parameters.
   Adding the AP method to the combination of CMB+BAO+SNIa+$H_0$ reduces the contour area by about 50\%. %DIF > significantly reduced the area of contour.
   Systematics does not have significant influence on the results.%DIF >  are less affected by systematics effect.
   %DIF > If we discard the systematics correction, there is half-$\sigma$ shift of the AP alone contour.
   %DIF > The combined constraints are less affected.
   %DIF > This suggests that, for the data analysis of current galaxy surveys,
   %DIF > the systematic effects in our method is not significant.
   %DIF > This shows the power of the AP method in constraining dynamical dark energy.
   %DIF > Middle panel: $w_0-w_a$ likelihood contours using datasets of CMB+BAO, CMB+BAO+SNIa+$H_0$ and CMB+BAO+AP+$H_0$.
   %DIF > When combined with CMB+BAO, AP method yields much tighter constraints than SNIa.
   %DIF > Right panel: Redshift evolution of $w(z)$, with/without adding the AP constraint.
   %DIF > Adding AP tightens the constraints and reduces the redshift evolution of $w$ (tilt of $w(z)$).
   }}
\end{figure*}
...
%    \includegraphics[height=8cm]{smu.eps}
   ...
   %DIF > Cosmological constraints on the CPL dark energy parametrization $w=w_0+w_a\frac{z}{1+z}$.
   %DIF > Left panel: 68.3\%, 95.4\% CL likelihood contours in the  $w_0-w_a$ plane.
   %DIF > Results are consistent with $\Lambda$CDM.
   %DIF > The constrained area is significantly reduced after combined with the AP method.
   %DIF > This shows the power of the AP method in constraining dynamical dark energy.
   %DIF > Middle panel: $w_0-w_a$ likelihood contours using datasets of CMB+BAO, CMB+BAO+SNIa+$H_0$ and CMB+BAO+AP+$H_0$.
   %DIF > When combined with CMB+BAO, AP method yields much tighter constraints than SNIa.
   \DIFaddFL{Derived redshift evolution of $w(z)$, with/without adding the AP constraint.
   ...
   There is no significant change in the results if we disgard the systematics correction procedure in the AP method analysis.
   }}
\end{figure*}



\DIFaddend %While the central values of parameters do not shift appreciably,
\DIFdelbegin \DIFdel{As a result of the additional constraint from our AP analysis, the error bars of $w_0$ and $w_a$ are reduced by 38.4}\DIFdelend %DIF > Covariance matrix 
\DIFaddbegin 


\DIFadd{Figure \ref{fig_con} shows the 68.3}\DIFaddend \% and \DIFdelbegin \DIFdel{29.7\%}\DIFdelend \DIFaddbegin \DIFadd{95.4\% CL likelihood contours in the $\Omega_m-w_0$  and $w_0-w_a$ planes,
derived from the AP method alone, CMB+BAO, CMB+BAO+SNIa+$H_0$
and CMB+BAO+SNIa+$H_0$+AP}\DIFaddend , respectively.
%DIF <  $$-40\%$ by adding our method into the analysis.
\DIFdelbegin \DIFdel{It can be also noted that the central value of $w_a$ moved to zero appreciably}\DIFdelend \DIFaddbegin \DIFadd{The overlapping of different contours suggest that they are consistent with each other}\DIFaddend .

\DIFdelbegin \DIFdel{The marginalized constraints on $w_0-w_a$ are shown in the left panel of Fig.\ref{fig_con}.
Adding our new AP constraints significantly reduces the allowable parameterspace.
This can also be visualized as allowable bands in $w(z)$ as can be seen in the right panel of Fig.
\ref{fig_con}}\DIFdelend \DIFaddbegin \DIFadd{Using AP method alone, we can lead to effective constraints on the parameter space in the $\Omega_m-w_0$ plan.
Due to the inclusion of one extra free parameter, 
the constrained area becomes larger than $w$CDM result \mbox{%DIFAUXCMD
\citep{Li2016}}%DIFAUXCMD
.
Given the current galaxy surveys it is difficult to effectively constrain $w_a$ using only one technique.
We have to combine various techniques}\DIFaddend .

\DIFdelbegin \DIFdel{The derived cosmological constraints are fully consistent with a cosmological constant dark energy component having no evolution.
%DIF < \cite{Li2016} 
Adding the new AP method to the }\DIFdelend \DIFaddbegin \DIFadd{Combinig the four external techniques, i.e. }\DIFaddend CMB+BAO+SNIa+$H_0$\DIFdelbegin \DIFdel{constraints reduces the statistical errors on many parametersby $\sim$30-40\% 
and reduces the $w_0-w_a$ (95.4\%CL) contour }\DIFdelend \DIFaddbegin \DIFadd{, leads to effective constraints on all parameters.
The statistical mean values and 68.3\% uncertainties of these parameters are
}\begin{eqnarray}
&\DIFadd{\Omega_m = 0.309 \pm 0.010,}\\
&\DIFadd{w_0 = -0.938 \pm 0.109,}\\
&\DIFadd{w_a = -0.38 \pm 0.41,
}\end{eqnarray}
\DIFadd{while adding our AP method to this combination further tightens the constraints and yield to
}\begin{eqnarray}
&\DIFadd{\Omega_m = 0.301 \pm 0.008,}\\
&\DIFadd{w_0 = -1.042 \pm 0.067,}\\
&\DIFadd{w_a = -0.07 \pm 0.29.
}\end{eqnarray}
\DIFadd{The error bars are dramatically reduced by 30-40\%.
The contour areas are reduced }\DIFaddend by 50\%\DIFdelbegin \DIFdel{.
This constitutes a significant increase in the }\DIFdelend \DIFaddbegin \DIFadd{, i.e., the dark energy }\DIFaddend figure of merit \DIFdelbegin \DIFdel{constraints on $(w_0, w_a)$.
This }\DIFdelend \DIFaddbegin \DIFadd{was improved by 100\%. 
Notice that the AP constraints come from the BOSS DR12 data, which is already used in the BAO analysis.
So the }\DIFaddend doubling of the figure of merit comes at no additional cost or alteration to data size,
thus greatly improving the overall cost-benefit \DIFdelbegin \DIFdel{consideration }\DIFdelend of large cosmological redshift surveys.


\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{It can be also noted that, after adding the AP method, the central value of $w_a$ moved to zero appreciably.
...
%    \includegraphics[height=8cm]{smu.eps}
   ...
   \DIFaddFL{Robustness test of the results.
...
\DIFadd{To see the effect of systematic effect, in Figure \ref{fig_con,fig_wz} we also plot the AP alone and CMB+BAO+SNIa+$H_0$+AP constraints 
...
\DIFadd{Finally, we }\DIFaddend performed a test of all the possible variables in our methodology.
\DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig_contest} shows that 
the result is unaffected by the line-of-sight $\mu$-cut, 
the \DIFdelbegin \DIFdel{number of angular bins, 
the }\DIFdelend range of radial integration\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, 
}\DIFaddend the choice of fiducial cosmology \DIFaddbegin \DIFadd{in the mapping of $\xi(s,\mu)$,
...
(the upper-left boundary is determined by the CMB+BAO+SNIa+$H_0$ constraints, so it is unaffected)}\DIFaddend .
...
\DIFdelbegin \DIFdel{The most striking point to note in our test is thatthe 
shift in contour position }\DIFdelend \DIFaddbegin \DIFadd{This further justifies our conclusion that, the effect of systematics }\DIFaddend is not significant \DIFdelbegin \DIFdel{even when %DIF < we do not include the systematic correction.
the systematics are not corrected.
%DIF < This is reassuring, since we may have worried that {\it cosmological dependence} 
%DIF < of the systematic correction could influence the final result. 
This indicates that, for the data analysis of current galaxy surveys,
the systematic effects in our method may not be important.
But it remains to be seen if this is true for future galaxy surveys,
or when the cosmology dependence of the systematics effects is taken into account}\DIFdelend \DIFaddbegin \DIFadd{in this analysis}\DIFaddend .
...
the redshift dependence of anisotropic galaxy clustering\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Li2016}}%DIFAUXCMD
}\DIFaddend .
...
In this analysis, \DIFaddbegin \DIFadd{we find }\DIFaddend the systematic effects \DIFdelbegin \DIFdel{and the covariance matrices }\DIFdelend \DIFaddbegin \DIFadd{do not significantly affect the derived cosmological constraints.
But it remains to be seen if this is true for future galaxy surveys,
or when the cosmology dependence of the systematics effects is taken into account.
Espeically, the systematic effects }\DIFaddend are estimated using simulations performed in a certain cosmology.
\DIFdelbegin \DIFdel{Their cosmological dependence needs }\DIFdelend \DIFaddbegin \DIFadd{The cosmological dependence remains }\DIFaddend to be investigated in future works\DIFdelbegin \DIFdel{to guarantee the robustness of the method}\DIFdelend .


\DIFdelbegin \DIFdel{Combining }\DIFdelend \DIFaddbegin \DIFadd{In this analysis, combining }\DIFaddend our method with the CMB+SNIa+BAO+$H_0$ datasets,
...
\DIFaddbegin \DIFadd{This indicates the great power of the method in constraining the cosmic expansion history and probing the properties of dark energy.
}\DIFaddend We expect the method will play an important role in deriving cosmological constraints from future spectroscopic galaxy surveys.
...
%DIF < \bibitem[Jackson (1972)]{FOG}
%DIF < Jackson, J., 1972, MNRAS, 156, 1
\DIFaddbegin \bibitem[Jackson (1972)]{FOG}
\DIFadd{Jackson, J., 1972, MNRAS, 156, 1
}\DIFaddend 
...
%DIF < \bibitem[Kaiser (1987)]{Kaiser1987}
%DIF < Kaiser, N. 1987, MNRAS, 227, 1
\DIFaddbegin \bibitem[Kaiser (1987)]{Kaiser1987}
\DIFadd{Kaiser, N. 1987, MNRAS, 227, 1
}\DIFaddend 
...
\DIFadd{Klypin, A., Yepes, G., Gottlober, S., Prada, F., \& Hess, S. 2016,
MNRAS, 457, 4340%DIF > 
}\DIFaddend 

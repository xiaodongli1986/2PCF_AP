...
% We improve the methodology of \citep{Li2016} by significantly reducing its computation time, 
% which enables us to explore three or more cosmological parameters.
% We applied the improved method on the Chevallier-Polarski-Linder parametrization of dark energy, $w=w_0+w_a\frac{z}{1+z}$.
...
% We carefully checked the robustness of the results.
...
% The number counts are measured in the fiducial cosmology in a uniform grid (blue dashed.
%  Just for illustration; the grid adopted in real analysis is much denser).
% In the four cosmologies with $(\Omega_m,w) = (0.31,-1.5),\ (0.31,-0.5),\ (0.41,-1.0)\ \rm and\ (0.21,-1.0)$, 
% %due to the distortion of geometry,
% the distribution of number counts in $(s, \mu)$ space becomes the red solid grid ({\bf Check}).
% We utilize the mapping realtion to approximate the 2pCF in vaiours cosmologies, without re-conducting the 2pCF statistics. 
...
% and approximate 2pCFs (blue solid) 2pCFs matches each other quite well.
%   }
%\end{figure*}

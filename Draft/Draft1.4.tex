...
We use 1,133,326 galaxies Sloan Digital Sky Survey Data Release 12 galaxies, spanning the redshift range $0.15<z<0.69$.
...
We improve the methodology of Li et al. to efficiently explore three or more cosmological parameters.
...
\begin{figure*}
   ...
   A patch of BOSS DR12 glaxies located in the fan shape region $\rm 140^\circ < RA < 170 ^\circ$, $\rm 10^\circ < DEC < 13 ^\circ$.
...
The constraining power is usually quantified by the DETF (Dark Energy Task Force) \citep{DETF} figure of merit, 
...
Figure \ref{fig_fan} shows the positions of a patch of 10,976 BOSS DR12 glaxies,
...
Systematics effects are estimated using mock catalogues derived from Horizon Run 4 (HR4) \citep{HR4},
...
They have been adopted for statistical analysis of BOSS data in a series of works (see \cite{Alam2016} and references therein).
...
%   \includegraphics[width=10.5cm]{fig_comp.eps}
 ...
 %due to the distortion of geometry,
...
\section{Cosmoligical constraints}
...
%    \includegraphics[height=8cm]{smu.eps}
   ...
   Adding the AP method to the combination of CMB+BAO+SNIa+$H_0$ reduces the contour area by about 50\%. %significantly reduced the area of contour.
...
Using AP method alone, we can lead to effective constraints on the parameter space in the $\Omega_m-w_0$ plan.
Due to the inclusion of one extra free parameter, 
the constrained area becomes larger than $w$CDM result \citep{Li2016}.
...
Combinig the four external techniques, i.e. CMB+BAO+SNIa+$H_0$, leads to effective constraints on all parameters.
...
thus greatly improving the overall cost-benefit of large cosmological redshift surveys.


It can be also noted that, after adding the AP method, the central value of $w_a$ moved to zero appreciably.
...
To see the effect of systematic effect, in Figure \ref{fig_con,fig_wz} we also plot the AP alone and CMB+BAO+SNIa+$H_0$+AP constraints 
...
In this analysis, we find the systematic effects do not significantly affect the derived cosmological constraints.
But it remains to be seen if this is true for future galaxy surveys,
or when the cosmology dependence of the systematics effects is taken into account.
Espeically, the systematic effects are estimated using simulations performed in a certain cosmology.
